...
class Versions
{
    ...
    public static function changelog()
    {
        ...
        $url = 'https://api.github.com/repos/nuvisaccounting/nuvisaccounting/releases';
    ...
    public static function all($modules = null)
    {
        return Cache::remember('versions', Date::now()->addHours(6), function () use ($modules) {
            ...
            // Check core first
            try {
                $url = 'core/version/' . $info['nuvisaccounting'] . '/' . $info['php'] . '/' . $info['mysql'] . '/' . $info['companies'];
            } catch (\Exception $e) {
                // Handle exception
            ...
            foreach ($modules as $module) {
                ...
                if (! $module instanceof \NuvisAccounting\Module\Module) {
                    continue;
                }
            ...
            $versions['core'] = static::getLatestVersion($url, $info['nuvisaccounting']);
            ...
            foreach ($modules as $module) {
                ...
                $url = 'apps/' . $alias . '/version/' . $version . '/' . $info['nuvisaccounting'];
